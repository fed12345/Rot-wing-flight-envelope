...
class LogParser(object):
    ...
    def plot_variable(self, msg_name, variable_names = [], idx = []):
        ...
        plt.legend()
    def store_logs(self, output_directory):
        """Function that compresses log dictionaries as binary

        Args:
            output_directory (path): Directory to where the logs will be stored
        """        
        count = 0
        name = ''
        for e in self.data_path[::-1]:
            if e == '/':
                count += 1
                e = '_'
            if count == 3:
            ...
            name += str(e)
        name = name[::-1]
        with open(output_directory+'/' +name[:-5]+ '.pkl', 'wb') as p:
